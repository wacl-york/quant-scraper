...
def main():
    ...
    # Base calls
    scrape_call = ["quant_convert_purpleair", "--html", email_fn]
    preprocess_call = ["quant_preprocess"]
    email_call = ["quant_email", "--file", email_fn]

    # Always pass in date
    preprocess_call.extend(["--date", parse_date])
    # TODO How to handle emails? can't use the send_email script since it has
    # hardcoded a Subject line with QUANT scraping summary and date!
    # Instead should have library code that can use in the CLI script for the
    # PurpleAir Conversion?
